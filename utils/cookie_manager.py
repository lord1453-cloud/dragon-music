...
def is_user_cookie_valid(cookie_path: Optional[str] = COOKIES_FILE) -> bool:
    """
            ...
            for line in f:
                ...
                if len(parts) >= 7:
                    ...
                    try:
                        ...
                        pass
        # Eğer çerezlerin yarısından fazlası eskiyse geçersiz say
        if total_cookies > 0 and expired_count >= total_cookies * 0.5:
            return False
        return total_cookies > 0
    except Exception as e:
        logger.debug(f"Kullanıcı çerez kontrol hatası: {e}")
        return False
...
async def _refresher_loop():
    ...
    while True:
        try:
            ...
            if not is_user_cookie_valid(COOKIES_FILE):
                await refresh_visitor_cookies(GUEST_COOKIES_FILE)
